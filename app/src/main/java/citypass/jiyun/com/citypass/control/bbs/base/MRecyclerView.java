package citypass.jiyun.com.citypass.control.bbs.base;
...
import citypass.jiyun.com.citypass.control.bbs.refresh.GifRfreshHeader;
import citypass.jiyun.com.citypass.control.bbs.refresh.LoadingMoreFooter;
...
public class MRecyclerView extends RecyclerView {
    ...
    private MRecyclerView.WrapAdapter mWrapAdapter;
    private float mLastY = -1;
    private static final float DRAG_RATE = 3;
    private MRecyclerView.LoadingListener mLoadingListener;
    ...
    private final RecyclerView.AdapterDataObserver mDataObserver = new MRecyclerView.DataObserver();
    ...
    public void setAdapter(Adapter adapter) {
        mWrapAdapter = new MRecyclerView.WrapAdapter(adapter);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    ...
                    && !isNoMore
                    && mRefreshHeader.getState() < GifRfreshHeader.STATE_REFRESHING) {
                isLoadingData = true;
                if (mFootView instanceof LoadingMoreFooter) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisibleHeight() > 0 && mRefreshHeader.getState() < GifRfreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
    ...
    private class WrapAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == TYPE_REFRESH_HEADER) {
                return new MRecyclerView.WrapAdapter.SimpleViewHolder(mRefreshHeader);
            } else if (isHeaderType(viewType)) {
                return new MRecyclerView.WrapAdapter.SimpleViewHolder(getHeaderViewByType(viewType));
            } else if (viewType == TYPE_FOOTER) {
                return new MRecyclerView.WrapAdapter.SimpleViewHolder(mFootView);
            }
    ...
    public void setLoadingListener(MRecyclerView.LoadingListener listener) {
    ...
    public void onScrolled(int dx, int dy) {
        ...
        if (mLoadingListener != null) {
            mLoadingListener.onRvScrolled(dx, dy);
        }
    }

    public interface LoadingListener {
        void onRvScrolled(int dx, int dy);
